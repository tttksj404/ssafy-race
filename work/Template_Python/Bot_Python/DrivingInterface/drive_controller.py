...
class DrivingController():
    ...
    def __init__(self):
        ...
        self.client.setResetLocation(json_pos.x_val + 0.01, json_pos.y_val + 0.01, json_pos.z_val, self.player_name)
    ...
    def run(self):
        ...
        penalty_count = 0
        max_speed = 0.0
        run_start_millis = self.get_current_milli_time()
        ...
        while self.game_playing(self.client):
            ...
            max_speed = max(max_speed, sensing_info.speed)
            ...
            if self.half_road_limit < distance_from_center:
                if abs(sensing_info.speed) > 40:
                    ...
                    #    car_controls.throttle = 1
                    # Counted in telemetry; avoid flooding local test logs.
                else:
            ...
            if now_time_millis - telemetry_stamp >= 2000:
                telemetry_stamp = now_time_millis
                near_obstacles = sensing_info.track_forward_obstacles[:3] if sensing_info.track_forward_obstacles else []
                print("[Telemetry] progress={:.2f} speed={:.1f} max_speed={:.1f} middle={:.2f} "
                      "angle={:.1f} steer={:.3f} throttle={:.2f} brake={:.2f} collisions={} penalties={} obs={}"
                      .format(sensing_info.lap_progress, sensing_info.speed, max_speed,
                              sensing_info.to_middle, sensing_info.moving_angle,
                              car_controls.steering, car_controls.throttle, car_controls.brake,
                              collision_count, penalty_count, near_obstacles))
        ...
        run_elapsed = (self.get_current_milli_time() - run_start_millis) / 1000.0
        print("[Telemetry] finished return_code={} elapsed={:.2f}s max_speed={:.1f} collisions={} penalties={}"
              .format(return_code, run_elapsed, max_speed, collision_count, penalty_count))
    ...
    def getPlayerName(self, json_data):
        ...
        try:
            ...
            if ('Vehicles' in json_data):
                vehicle_names = [key for key, value in sorted(json_data['Vehicles'].items())]
                for key in vehicle_names:
                    if self.player_name == "" and airsim.CarClient().isApiControlEnabled(key) == False:
                        self.player_name = key
                        break
                if self.player_name == "" and vehicle_names:
                    self.player_name = vehicle_names[0]
    ...
    def getJosnfile(self):
        ...
        if path == "":
            ...
            candidates = [
                ...
                os.path.expanduser("~/Documents/Airsim/settings.json"),
                "C:\\Users\\SDS\\Documents\\AirSim\\settings.json",
            ]
    ...
    def getStartPos(self, json_data):
        if self.player_name != "":
            ...
            if "Yaw" in json_car:
                Yaw = json_car["Yaw"]
            pos_setting = airsim.Vector3r(float(json_car["X"]), float(json_car["Y"]), Yaw)
        else:
