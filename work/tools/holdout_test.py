...
OUT=ROOT/"work/experiments/holdout.txt"; open(OUT,"w").write("map fin elapsed col maxprog\n")
def run(idx,name):
    e={**os.environ,"SSAFY_GENERAL_ONLY":"1"}
    p=subprocess.run(["./work/run_experiment.sh",idx,f"holdout_{name}"],cwd=str(ROOT),text=True,
